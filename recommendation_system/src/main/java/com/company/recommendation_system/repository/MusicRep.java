...
public interface MusicRep extends JpaRepository<Music, Long> {
    ...
    @Query(value = "select * from tb_music where status = 'ACTIVE' and author like %:author% order by name desc",nativeQuery = true)
    ...
    @Query(value = "select * from tb_music where status = 'ACTIVE' and name like %:name% order by name desc",nativeQuery = true)
    List<Music> findAllByName(String name);

    @Query(value = "select * from tb_music order by number_of_plays desc", nativeQuery = true)
    List<Music> findAllByPopular();

    @Query(value = "select * from tb_music order by date_of_issue desc", nativeQuery = true)
    List<Music> findAllByNew();
}
